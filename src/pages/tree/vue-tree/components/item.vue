<!--
 * @Author: wuchen
 * @Date: 2019-09-09 14:54:02
 * @LastEditors: wuchen
 * @LastEditTime: 2019-09-10 21:01:46
 * @Description: 
...
<template>
  <li class="tree" :class="{'sub_tree_all': model.child.length == 0? true : false }">
    ...
    <ul>
      <tree-node v-for="(subModel , index) in model.child" :model="subModel" :ids="ids" :key="index" :root="root" :status="itemStatus" :withoutParents="withoutParents" :options="options" @onChildChange="onChildChange"></tree-node>
    </ul>
  </li>
</template>
<script>
        ...
        if (this.ids.indexOf(this.model.id) === -1) {
          ...
          if (!this.options.withParents) {
            if (this.model.child && this.model.child.length == 0) {
              this.$set(
                _this.withoutParents,
                _this.withoutParents.length,
                _this.model.id
              );
            }
          }
