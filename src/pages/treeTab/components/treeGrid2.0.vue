                ...
                <tr>
                    <th v-for="(column,index) in cloneColumns">
                        <label v-if="column.type === 'selection'">
                                <input class="inputSelect" type="checkbox" v-model="checks" @click="handleCheckAll">
                            </label>
                        <label v-else>
                                {{ renderHeader(column, index) }}
                                <!-- <span class="ivu-table-sort" v-if="column.sortable">
                                    <i class="el-icon-arrow-up" :class="{on: column._sortType === 'asc'}" @click.native="handleSort(index, 'asc')"></i>
                                    <i class="el-icon-arrow-down" :class="{on: column._sortType === 'desc'}" @click.native="handleSort(index, 'desc')"></i>
                                </span> -->
                            </label>
                    </th>
                </tr>
            ...
            <tbody>
                <tr v-for="(item,index) in initItems" :key="item.id" v-show="show(item)" :class="{'child-tr':item.parent}">
                    <td v-for="(column,snum) in columns" :key="column.key" :style=tdStyle(column)>
                        <label v-if="column.type === 'selection'">
                                <input type="checkbox" class="inputSelect" :value="item.id" v-model="checkGroup" @click="handleCheckClick(item,$event,index)">
                            </label>
                        <div v-if="column.type === 'action'">
                            <el-button :type="action.type" size="small" @click="RowClick(item,$event,index,action.text)" v-for='action in (column.actions)' :key="action.text">{{action.text}}</el-button>
                        </div>
                        <label @click="toggle(index,item)" v-if="!column.type">
                                <span v-if='snum==iconRow()'>
                                    <i v-html='item.spaceHtml'></i>
                                    <i class="el-icon-arrow-down ivu-icon" v-if="item.children&&item.children.length>0" :class="{'ivu-icon-plus-circled':!item.expanded,'ivu-icon-minus-circled':item.expanded }"></i>
